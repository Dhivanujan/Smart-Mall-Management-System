		...
		<div className="panel-page">
			...
			<div className="metric-grid" style={{ gridTemplateColumns: "repeat(auto-fit, minmax(140px, 1fr))", marginBottom: "1.5rem" }}>
				{Object.entries(statusColors).map(([status, color]) => (
					<div key={status} className="metric-card" style={{ cursor: "pointer", borderColor: filter === status ? color : undefined }}
			...
			<div style={{ display: "flex", gap: "0.5rem", marginBottom: "1.5rem", flexWrap: "wrap" }}>
				...
				{Object.keys(statusColors).map((s) => (
					<button key={s} className={`btn ${filter === s ? "btn-primary" : ""}`} onClick={() => setFilter(s)} style={{ textTransform: "capitalize" }}>
						{s.replace("_", " ")}
					</button>
				))}
			</div>
					...
					{complaints.map((c) => (
						<div key={c.id} className="section-card">
							...
							<div style={{ display: "flex", gap: "0.5rem", flexWrap: "wrap" }}>
								{c.status === "open" && (
									<button className="btn" style={{ fontSize: "0.8rem" }} onClick={() => handleStatusChange(c.id, "in_progress")}>
										Start Working
									</button>
								)}
								...
								{c.status === "resolved" && (
									<button className="btn" style={{ fontSize: "0.8rem" }} onClick={() => handleStatusChange(c.id, "closed")}>
										Close
									</button>
								)}
							</div>
