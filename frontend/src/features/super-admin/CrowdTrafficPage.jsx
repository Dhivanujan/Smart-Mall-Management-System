...
export const CrowdTrafficPage = () => {
    ...
    if (!data)
        return <p>No crowd data available</p>;
    ...
    return (<div className="panel-page">
			...
			{data.congestion_alerts.length > 0 && (<div className="section-card" style={{ marginBottom: "1.5rem", border: "1px solid var(--color-danger)" }}>
					<h2 className="section-title" style={{ color: "var(--color-danger)" }}>Congestion Alerts</h2>
					{data.congestion_alerts.map((alert, i) => (<div key={i} className="alert-item" style={{
        ...
        ].map((l) => (<span key={l.label} style={{ display: "flex", alignItems: "center", gap: "0.3rem", fontSize: "0.8rem" }}>
