...
export const SuperAdminAdminsPage = () => {
    ...
    useEffect(() => {
        ...
        const load = async () => {
            try {
                const res = await apiClient.get("/api/v1/admin/super/admins");
                if (cancelled)
                    return;
			...
			{!loading && !error && (<>
					...
					<div className="filter-bar animate-fade-in-up stagger-2" style={{ marginTop: "1.25rem" }}>
						<div className="search-input-wrapper">
							...
							<input type="text" placeholder="Search by name, username or mall…" value={search} onChange={(e) => setSearch(e.target.value)}/>
						</div>
					</div>
