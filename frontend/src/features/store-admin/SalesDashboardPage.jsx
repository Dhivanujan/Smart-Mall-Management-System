...
export const SalesDashboardPage = () => {
    ...
    return (<div className="panel-page">
			...
			<div style={{ display: "flex", gap: "0.5rem", marginBottom: "1.5rem" }}>
				{["daily", "weekly", "monthly"].map((p) => (<button key={p} className={`btn ${period === p ? "btn-primary" : ""}`} onClick={() => setPeriod(p)} style={{ textTransform: "capitalize" }}>
						{p}
					</button>))}
			</div>
			...
			{sales && (<>
					<div className="metric-grid" style={{ gridTemplateColumns: "repeat(auto-fit, minmax(min(100%, 180px), 1fr))" }}>
						...
						{customers && (<div className="metric-card">
								<span className="metric-icon">👥</span>
					...
					<div className="section-card" style={{ marginTop: "2rem" }}>
						...
						<div style={{ display: "flex", alignItems: "flex-end", gap: "4px", height: "200px", paddingTop: "1rem" }}>
							{sales.daily_breakdown.map((d, i) => (<div key={i} style={{ flex: 1, display: "flex", flexDirection: "column", alignItems: "center", gap: "4px" }}>
			...
			{customers && (<div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "1.5rem", marginTop: "2rem" }}>
					<div className="section-card">
						<h2 className="section-title">Customer Segments</h2>
						{customers.customer_segments.map((seg) => (<div key={seg.segment} style={{ marginBottom: "0.75rem" }}>
					...
					<div className="section-card">
						...
						{customers.shopping_patterns.map((vp) => (<div key={vp.hour} style={{ display: "flex", alignItems: "center", gap: "1rem", marginBottom: "0.5rem" }}>
