				...
				<>
					<div className="metric-grid" style={{ gridTemplateColumns: "repeat(auto-fit, minmax(180px, 1fr))" }}>
						...
						<div className="metric-card">
							...
							<span className="metric-value">{sales.total_orders.toLocaleString()}</span>
						</div>
						<div className="metric-card">
							<span className="metric-icon">📊</span>
							<span className="metric-label">Avg Order Value</span>
							<span className="metric-value">${sales.average_order_value.toFixed(2)}</span>
						</div>
						{customers && (
							<div className="metric-card">
								<span className="metric-icon">👥</span>
								<span className="metric-label">Total Customers</span>
								<span className="metric-value">{customers.total_customers.toLocaleString()}</span>
							</div>
						)}
					</div>
				...
				<div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "1.5rem", marginTop: "2rem" }}>
					<div className="section-card">
						...
						{customers.segments.map((seg) => (
							<div key={seg.name} style={{ marginBottom: "0.75rem" }}>
								<div style={{ display: "flex", justifyContent: "space-between", marginBottom: "0.25rem", fontSize: "0.9rem" }}>
									<span>{seg.name}</span>
									<span style={{ color: "var(--color-text-muted)" }}>{seg.count} ({seg.percentage}%)</span>
								</div>
								<div className="zone-bar" style={{ height: "8px" }}>
									<div className="zone-bar-fill" style={{ width: `${seg.percentage}%` }} />
								</div>
							</div>
						))}
					</div>
					<div className="section-card">
						...
						{customers.visit_patterns.map((vp) => (
							<div key={vp.day} style={{ display: "flex", alignItems: "center", gap: "1rem", marginBottom: "0.5rem" }}>
								<span style={{ width: "40px", fontSize: "0.85rem", fontWeight: 600 }}>{vp.day}</span>
								<div className="zone-bar" style={{ flex: 1, height: "8px" }}>
									<div className="zone-bar-fill" style={{ width: `${(vp.avg_visits / 200) * 100}%` }} />
								</div>
								<span style={{ fontSize: "0.8rem", color: "var(--color-text-muted)", width: "30px", textAlign: "right" }}>{vp.avg_visits}</span>
							</div>
						))}
					</div>
				</div>
