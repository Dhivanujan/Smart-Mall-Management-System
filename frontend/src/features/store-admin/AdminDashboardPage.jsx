...
export const AdminDashboardPage = () => {
    ...
    return (<DashboardLayout title="Admin Panel" navItems={ADMIN_NAV}>
			...
			<div className="flex-between" style={{ flexWrap: "wrap", gap: "0.75rem" }}>
				...
				{lastRefresh && (<div className="refresh-indicator">
			...
			{/* Metric cards */}
			{!loading && !error && metrics && (<div className="metrics-grid animate-fade-in-up" style={{ marginTop: "1.5rem" }}>
			...
			{!loading && !error && snapshot && (<div className="dashboard-grid animate-fade-in-up stagger-2" style={{ marginTop: "1.5rem" }}>
					{/* Footfall by zone */}
					<div className="section-card">
						...
						{Object.entries(snapshot.footfall.by_zone).map(([zone, value]) => (<div className="zone-bar" key={zone}>
					...
					<div className="section-card">
						...
						{snapshot.alerts.length === 0 && (<div className="empty-state" style={{ padding: "1.5rem 0" }}>
								<div className="empty-state-icon">✅</div>
								<h3>All clear</h3>
								<p>No active alerts at this time.</p>
							</div>)}
						{snapshot.alerts.map((alert) => (<div className={`alert-item ${alert.severity.toLowerCase() === "critical" ? "critical" : alert.severity.toLowerCase() === "warning" ? "warning" : "info"}`} key={alert.id}>
			...
			{!loading && !error && stores.length > 0 && (<div className="section-card animate-fade-in-up stagger-3" style={{ marginTop: "1.5rem" }}>
			...
			{!loading && !error && storeMetrics.length > 0 && (<div className="section-card animate-fade-in-up stagger-4" style={{ marginTop: "1.5rem" }}>
					<div className="flex-between" style={{ marginBottom: "1rem" }}>
					...
					<div className="data-table-wrapper">
						<table className="data-table">
							...
							<tbody>
								{storeMetrics.map((store) => (<tr key={store.store_id}>
										<td style={{ fontWeight: 500 }}>{store.name}</td>
