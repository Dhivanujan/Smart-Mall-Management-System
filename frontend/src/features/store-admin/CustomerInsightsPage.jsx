			...
			<div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "1.5rem" }}>
				<div className="section-card">
					...
					<div style={{ display: "flex", gap: "0.5rem", marginBottom: "1.5rem", height: "20px", borderRadius: "10px", overflow: "hidden" }}>
						{data.customer_segments.map((seg, i) => (<div key={seg.segment} style={{
                flex: seg.count,
                background: segColors[i % segColors.length],
                minWidth: "2px",
            }} title={`${seg.segment}: ${seg.count}`}/>))}
					</div>
					{data.customer_segments.map((seg, i) => (<div key={seg.segment} style={{ display: "flex", alignItems: "center", gap: "0.75rem", marginBottom: "0.75rem" }}>
				...
				<div className="section-card">
					...
					<div style={{ display: "flex", alignItems: "flex-end", gap: "8px", height: "200px" }}>
						{data.shopping_patterns.map((vp) => (<div key={vp.hour} style={{ flex: 1, display: "flex", flexDirection: "column", alignItems: "center", gap: "4px", height: "100%" }}>
